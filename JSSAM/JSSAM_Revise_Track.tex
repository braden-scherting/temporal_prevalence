...
%% Title
\title{Bayesian data integration for \sout{small area} estimation of pathogen prevalence dynamics from pooled\sout{ and individual} data}
...
\begin{align}
    \begin{split}
        Y_{t_i, j}|\pi_{t_i,j} &\overset{\mathrm{iid}}{\sim} \text{Bernoulli}(\pi_{t_i,j}) \\
...
\begin{align}
    \begin{split}
        Y_{t_i}|\pi_{t_i}&\overset{\mathrm{iid}}{\sim} \text{Binomial}(k_{t_i}, \pi_{t_i})\\
        \pi_{t_i} &= 1- (1-p_{t_i})^{m_{t_i}},
    \end{split}
\end{align}
where $Y_{t_i}\in\{0,1,\dots,k_{t_i}\}$ represents the number of pools that test positive at $t_i$. \sout{Synthetic} \red{Simulated} examples follow this framework. This model is not appropriate when pools are different sizes, as is the case when the pool size does not divide the number of individual samples. 
...
\begin{split} \label{hier}
    ...
    W_t|C &\sim \mathcal{GP}(\mathbf{0}, C) \\
    \mu &\sim p(\mu) \\
    \theta &\sim p(\theta).
\end{split}
\end{align}
...
\begin{align*}
    z_{t_i}|p_{t_i} \overset{\mathrm{iid}}{\sim} \text{Binomial}(k_{t_i}, p_{t_i}),
\end{align*}
where $k_{t_i}$ is now the number of individuals tested at $t_i$ and $z_{t_i}\in \{1,\dots,k_{t_i}\}$ is the observed number of infected individuals. 
...
\begin{split} \label{eq:di}
    z_{t_i,j}|p_{t_i,j} & \overset{\mathrm{iid}}{\sim}\text{Binomial}(k_{t_i,j}, p_{t_i,j})\\
    y_{t_i}|\pi_{t_i} &\overset{\mathrm{iid}}{\sim} \text{Binomial}(k_{t_i}, \pi_{t_i}) \\
    \pi_{t_i} &= 1 - (1 - p_{t_i})^{m_{t_i}} \\
    p_{t_i} &= \phi(W_{t_i} + \mu) \\
        p_{t_i,j} &= \phi(W_{t_i} + \mu_j) \\
    W_t|C &\sim \mathcal{GP}(\mathbf{0}, C) \\
     \mu_j|\mu &\overset{\mathrm{iid}}{\sim} N(\mu, \sigma^2_\mu) \\
     \mu &\sim p(\mu) \\
    \theta &\sim p(\theta).
\end{split}
\end{align}
...
% \url{https://github.com/braden-scherting/temporal_prevalence}
